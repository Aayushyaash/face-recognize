...
class TestFace:
    ...
    def test_similarity_with_another_face(self):
        ...
        embedding2 = np.array([0.8, 0.6, 0.0])  # Also normalized (sqrt(0.8^2 + 0.6^2) = 1.0)
        
        bbox = BoundingBox(x1=0, y1=0, x2=10, y2=10)
        landmarks = np.zeros((5, 2))  # 5 landmarks with 2 coordinates each
        
        face1 = Face(embedding=embedding1, bbox=bbox, confidence=0.9, landmarks=landmarks)
        face2 = Face(embedding=embedding2, bbox=bbox, confidence=0.8, landmarks=landmarks)
        
        # Cosine similarity of [1,0,0] and [0.8,0.6,0] should be 1*0.8 + 0*0.6 + 0*0 = 0.8
    ...
    def test_similarity_to_embedding(self):
        ...
        face = Face(embedding=embedding1, bbox=bbox, confidence=0.9, landmarks=landmarks)
        
        # Cosine similarity of [1,0,0] and [0.6,0.8,0] should be 1*0.6 + 0*0.8 + 0*0 = 0.6
