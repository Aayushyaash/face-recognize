...
def test_person_record_to_dict() -> None:
...
def test_person_record_from_dict() -> None:
    ...
    assert record.embedding.shape == (512,)
    assert record.embedding.dtype == np.float32
    expected_array = np.array(embedding_list, dtype=np.float32)
    actual_embedding = record.embedding
    result = np.array_equal(actual_embedding, expected_array)
    assert result


def test_person_record_embedding_not_auto_copied() -> None:
    """Test that embedding is not automatically copied
    when creating PersonRecord directly."""
