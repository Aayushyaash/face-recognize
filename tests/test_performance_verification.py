...
def test_performance_requirements_verification():
    ...
    with unittest.mock.patch.object(detector.model, 'get') as mock_get:
        ...
        assert avg_time < 100, f"Average face detection time {avg_time:.2f} ms exceeds requirement of <100ms"
...
def test_latency_optimization_verification():
    ...
    with unittest.mock.patch.object(detector.model, 'get') as mock_get:
        ...
        for h, w, c in image_sizes:
            ...
            assert execution_time_ms < 500, f"Processing for {w}x{h} image took {execution_time_ms:.2f} ms"
