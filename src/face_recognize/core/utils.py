...
def compute_pairwise_iou(bboxes1: List[BoundingBox], bboxes2: List[BoundingBox]) -> np.ndarray:
    ...
    ious = inter_areas / np.maximum(union_areas, 1e-9)  # Avoid division by zero
    
    return ious


def find_best_matches(iou_matrix: np.ndarray, threshold: float = 0.3) -> List[Tuple[int, int]]:
